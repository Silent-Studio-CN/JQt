...
```powershell
# 下载 jqt-0.7.1-Universal-Kit-windows-x64.zip → 解压
# 运行注意：jqt.dll 依赖 Qt6*.dll，需把 lib 目录加入 DLL 搜索路径
# （cd 到 lib 目录，或把 lib 加入 PATH）——-Djava.library.path 只定位 jqt.dll 本身
cd lib
java -Djava.library.path=. -cp "jqt-0.7.1-Universal-Kit.jar;.." Hello
# 或：不切目录，用 PATH 方式
# $env:PATH = "$PWD\lib;$env:PATH"
# java -Djava.library.path=lib -cp "lib\jqt-0.7.1-Universal-Kit.jar;." Hello
```
...
## 📦 发布包（v0.7.1-Universal-Kit）

| 资产 | 平台 |
|------|------|
| `jqt-0.7.1-Universal-Kit.jar` | 全部（Java API） |
| `jqt-0.7.1-Universal-Kit-windows-x64.zip` | Windows x64 完整包（Qt 6.11.2 运行库） |
